  ...
  if (grenades.length === 0) {
    return (
      <div className="text-center py-12">
        <h3 className="text-xl font-medium text-gray-400">Раскидки для этой карты пока не добавлены</h3>
        <p className="mt-2 text-gray-500">Скоро здесь появятся полезные раскидки гранат</p>
      </div>
    )
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {grenades.map((grenade) => (
        <Link key={grenade.id} href={`/maps/${mapId}/grenades/${grenade.id}`} className="group">
          <div className="aspect-video relative overflow-hidden rounded-lg">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4">
              ...
              <h3 className="font-medium">{grenade.title}</h3>
            </div>
          </div>
        </Link>
      ))}
